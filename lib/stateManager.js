...
async function updateOrderStates(adapter, order, apiData) {
    ...
    // Detect status change — null means no previous state (fresh install), which
    // counts as a change so the user gets notified when an order first appears.
    const prevState = await adapter.getStateAsync(`${prefix}.status`);
    const prevStatus = prevState ? prevState.val : null;
    const statusChanged = prevStatus !== apiData.status;
